...
public class StudentsubjectResource {
    ...
    public ResponseEntity<List<StudentsubjectDTO>> getAllStudentsubjects(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Studentsubjects");
        Page<StudentsubjectDTO> page;
        if (eagerload) {
            page = studentsubjectService.findAllWithEagerRelationships(pageable);
        } else {
            page = studentsubjectService.findAll(pageable);
        }
