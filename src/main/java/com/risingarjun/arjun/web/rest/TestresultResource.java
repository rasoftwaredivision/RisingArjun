...
public class TestresultResource {
    ...
    public ResponseEntity<List<TestresultDTO>> getAllTestresults(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Testresults");
        Page<TestresultDTO> page;
        if (eagerload) {
            page = testresultService.findAllWithEagerRelationships(pageable);
        } else {
            page = testresultService.findAll(pageable);
        }
